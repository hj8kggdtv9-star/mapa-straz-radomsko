...
async function init({sb},box){
 ...
 let schemaReady=false;try{const{error}=await sb.from('vehicles').select('force_group').limit(1);schemaReady=!error}catch{}
 if(!schemaReady){console.warn('FIREMAP: structured external-force schema not active yet; legacy WOO/COO mode remains enabled.');return}
