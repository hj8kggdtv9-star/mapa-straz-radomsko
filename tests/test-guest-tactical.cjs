...
(async()=>{
 ...
 r=await a.from('incident_drawings').upsert([{id:'a'}]);assert.match(r.error.message,/odczytu/);assert.ok(!calls.some(c=>c.name==='external_tactical_write'));
 fail=true; r=await a.from('incidents').select('*');assert.ok(r.error);assert.equal(expired,1);
...
(async()=>{
 ...
 assert.ok(loaded.some(x=>x.startsWith('tactical-shared-viewer')),'viewer loads independently of failed layers and RPC');
 ...
 assert.equal(loaded.filter(x=>x.startsWith('tactical-shared-viewer')).length,1,'retry must not duplicate viewer');
