...
class MainActivity:AppCompatActivity(){
 ...
 @SuppressLint("SetJavaScriptEnabled") private fun setup(){web.settings.javaScriptEnabled=true;web.settings.domStorageEnabled=true;web.settings.databaseEnabled=true;web.settings.cacheMode=WebSettings.LOAD_CACHE_ELSE_NETWORK;web.settings.allowFileAccess=true;web.settings.mediaPlaybackRequiresUserGesture=false;web.webViewClient=object:WebViewClient(){override fun shouldInterceptRequest(v:WebView?,r:WebResourceRequest?):WebResourceResponse? { val url=r?.url?.toString() ?: return null; return OfflineStore.response(this@MainActivity,url) }};web.webChromeClient=object:WebChromeClient(){override fun onGeolocationPermissionsShowPrompt(origin:String?,cb:GeolocationPermissions.Callback?){if(ContextCompat.checkSelfPermission(this@MainActivity,Manifest.permission.ACCESS_FINE_LOCATION)==PackageManager.PERMISSION_GRANTED)cb?.invoke(origin,true,false)else{requestPermissions();cb?.invoke(origin,false,false)}}};web.addJavascriptInterface(Bridge(),"FiremapAndroid")}
