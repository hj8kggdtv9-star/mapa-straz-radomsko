...
export function makeHandler(clientFactory, env) {
 ...
 return async req=>{
  ...
  try {
   ...
   const {data:account,error:accountError}=await admin.from('firemap_accounts').select('role,enabled').eq('user_id',userData.user.id).single();
   if(accountError||!account?.enabled||account.role!=='SK')return reply(403,{error:'Tylko SK może utworzyć konto sztabu.'});
   ...
   const {error:profileError}=await admin.from('firemap_accounts').insert({user_id:created.user.id,role:'SZTAB',unit_name:name,enabled:true});
