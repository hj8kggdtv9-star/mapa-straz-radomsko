...
export function makeHandler(clientFactory, env) {
 ...
 return async req=>{
  ...
  try {
   ...
   admin=clientFactory(env('SUPABASE_URL'),env('SUPABASE_SERVICE_ROLE_KEY'),{auth:{persistSession:false,autoRefreshToken:false}});
   ...
   if(profileError)throw Error('Profile creation failed');
   ...
   if(cleanupFailed){console.error('FIREMAP sztab cleanup failed',{user_id:createdUser});return reply(500,{error:'Nie udało się dokończyć tworzenia sztabu ani potwierdzić wycofania zmian. Administrator musi sprawdzić konto przed ponowną próbą.',code:'PROVISIONING_CLEANUP_REQUIRED'});}
   return reply(500,{error:'Nie udało się utworzyć sztabu. Sprawdź połączenie i listę kont przed ponowną próbą.'});
  }
 };
}
