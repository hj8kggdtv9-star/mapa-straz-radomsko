...
-- Tylko aktywne konto SK może czytać kody i sesje zewnętrzne.
...
as $$
  select exists(select 1 from public.firemap_accounts a where a.user_id=auth.uid() and a.enabled and a.role='SK');
$$;
...
begin
  ...
  p_minutes := greatest(10, least(coalesce(p_minutes,120), 1440));
  ...
  loop
    ...
    if i>20 then raise exception 'Nie udało się wygenerować unikalnego kodu'; end if;
  end loop;
  insert into public.incident_join_codes(incident_id,code,expires_at,created_by)
  values(p_incident_id,v_code,v_exp,auth.uid()) returning id into v_id;
...
create or replace function public.external_force_join(
  p_code text,
  p_force_group text,
  p_specialist_group text,
  p_voivodeship text,
  p_county text,
  p_unit text,
  p_call_sign text,
...
as $$
declare
  c public.incident_join_codes%rowtype;
  s_token text;
  s_id uuid;
  v_id uuid;
  s_exp timestamptz;
...
begin
  ...
  if length(trim(coalesce(p_voivodeship,'')))<2 or length(trim(coalesce(p_county,'')))<2 or length(trim(coalesce(p_unit,'')))<2 or length(trim(coalesce(p_call_sign,'')))<1 or length(trim(coalesce(p_vehicle_type,'')))<1 then
    raise exception 'Uzupełnij dane zastępu';
  end if;

  select * into c from public.incident_join_codes
  ...
  select * into inc from public.incidents where id=c.incident_id and coalesce(active,true)=true;
  if inc.id is null then raise exception 'Zdarzenie zostało zakończone'; end if;

  s_token := encode(gen_random_bytes(24),'hex');
  s_exp := greatest(c.expires_at, now()+interval '12 hours');
  insert into public.external_force_sessions(
    incident_id,join_code_id,token_hash,force_group,specialist_group,origin_voivodeship,origin_county,origin_unit,call_sign,vehicle_type,expires_at
  ) values(
    c.incident_id,c.id,digest(s_token,'sha256'),p_force_group,nullif(p_specialist_group,''),trim(p_voivodeship),trim(p_county),trim(p_unit),trim(p_call_sign),trim(p_vehicle_type),s_exp
...
create or replace function public.external_force_update(
...
as $$
declare
  s public.external_force_sessions%rowtype;
begin
  if p_status not in ('DISPATCHED','ON_SCENE','RETURNING','BASE') then raise exception 'Nieprawidłowy status'; end if;
  if p_lat<-90 or p_lat>90 or p_lng<-180 or p_lng>180 then raise exception 'Nieprawidłowa pozycja'; end if;
  select * into s from public.external_force_sessions
  where token_hash=digest(coalesce(p_session_token,''),'sha256') and active=true and expires_at>now()
  limit 1;
  if s.id is null then raise exception 'Sesja wygasła'; end if;
  if not exists(select 1 from public.incidents i where i.id=s.incident_id and coalesce(i.active,true)=true) then
    update public.external_force_sessions set active=false where id=s.id;
    update public.vehicles set status='BASE',updated_at=now() where id=s.vehicle_id;
    raise exception 'Zdarzenie zostało zakończone';
  end if;

  update public.external_force_sessions set last_seen_at=now() where id=s.id;
  insert into public.vehicles(
    id,unit_name,call_sign,vehicle_type,status,lat,lng,accuracy,speed,heading,updated_at,incident_id,
    force_group,specialist_group,origin_voivodeship,origin_county,origin_unit,external_session_id
  ) values(
    s.vehicle_id,s.origin_unit,s.call_sign,s.vehicle_type,p_status,p_lat,p_lng,p_accuracy,p_speed,p_heading,now(),s.incident_id,
    s.force_group,s.specialist_group,s.origin_voivodeship,s.origin_county,s.origin_unit,s.id
  )
  on conflict(id) do update set
    unit_name=excluded.unit_name,call_sign=excluded.call_sign,vehicle_type=excluded.vehicle_type,status=excluded.status,
    lat=excluded.lat,lng=excluded.lng,accuracy=excluded.accuracy,speed=excluded.speed,heading=excluded.heading,
    updated_at=excluded.updated_at,incident_id=excluded.incident_id,force_group=excluded.force_group,specialist_group=excluded.specialist_group,
    origin_voivodeship=excluded.origin_voivodeship,origin_county=excluded.origin_county,origin_unit=excluded.origin_unit,external_session_id=excluded.external_session_id;
  return true;
end;
$$;
...
declare
  s public.external_force_sessions%rowtype;
begin
  select * into s from public.external_force_sessions where token_hash=digest(coalesce(p_session_token,''),'sha256') and active=true limit 1;
  if s.id is null then return false; end if;
  update public.external_force_sessions set active=false,last_seen_at=now() where id=s.id;
  update public.vehicles set status='BASE',updated_at=now() where id=s.vehicle_id;
  return true;
end;
$$;
...
begin
  if not public.firemap_is_sk() then raise exception 'Brak uprawnień SK'; end if;
  update public.incident_join_codes set active=false where active and expires_at<=now();
  with expired as (
    update public.external_force_sessions s
    set active=false
    where s.active and (s.expires_at<=now() or not exists(select 1 from public.incidents i where i.id=s.incident_id and coalesce(i.active,true)=true))
    returning s.vehicle_id
  )
...
begin
  if old.active is distinct from new.active and new.active=false then
    update public.incident_join_codes set active=false where incident_id=new.id;
    update public.external_force_sessions set active=false where incident_id=new.id and active=true;
    update public.vehicles set status='BASE',updated_at=now() where external_session_id in (select id from public.external_force_sessions where incident_id=new.id);
  end if;
  return new;
end;
$$;
...
create trigger firemap_close_external_sessions_on_incident
after update of active on public.incidents
for each row execute function public.firemap_close_external_sessions_on_incident();
